...
export class InsightsPanel extends Panel {
  ...
  public async updateInsights(clusters: ClusteredEvent[]): Promise<void> {
    ...
    // Build summarize options from AI flow settings (desktop + web).
    // Desktop keeps local paths active and treats paid cloud providers as optional.
    const aiFlow = getAiFlowSettings();
    const summarizeOpts: SummarizeOptions = {
      skipCloudProviders: !isDesktopRuntime() && !aiFlow.cloudLlm,
      skipPaidCloudProviders: isDesktopRuntime() && !aiFlow.cloudLlm,
