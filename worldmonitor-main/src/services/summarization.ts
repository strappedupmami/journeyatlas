...
export interface SummarizeOptions {
  ...
  skipPaidCloudProviders?: boolean; // true = skip paid cloud providers (Groq/OpenRouter) but keep local Ollama
...
function selectApiProviders(options?: SummarizeOptions): ApiProviderDef[] {
  if (options?.skipCloudProviders) {
    return [];
  }
  if (options?.skipPaidCloudProviders) {
    return API_PROVIDERS.filter((provider) => provider.provider === 'ollama');
  }
...
async function generateSummaryInternal(
  ...
  const providerChain = selectApiProviders(options);

  if (BETA_MODE) {
    const modelReady = mlWorker.isAvailable && mlWorker.isModelLoaded('summarization-beta');

    if (modelReady) {
      const totalSteps = 1 + providerChain.length;
      ...
      if (!options?.skipBrowserFallback) {
        ...
        if (browserResult) {
          const groqProvider = providerChain.find(p => p.provider === 'groq');
      ...
      if (providerChain.length > 0) {
        const chainResult = await runApiChain(providerChain, headlines, geoContext, undefined, onProgress, 2, totalSteps);
        if (chainResult) return chainResult;
      }
    } else {
      const totalSteps = providerChain.length + 2;
      ...
      if (providerChain.length > 0) {
        const chainResult = await runApiChain(providerChain, headlines, geoContext, undefined, onProgress, 1, totalSteps);
        if (chainResult) {
          return chainResult;
        }
      }
      ...
      if (mlWorker.isAvailable && !options?.skipBrowserFallback) {
        onProgress?.(providerChain.length + 1, totalSteps, 'Waiting for local AI model...');
        const browserResult = await tryBrowserT5(headlines, 'summarization-beta');
  ...
  const totalSteps = providerChain.length + 1;
  let chainResult: SummarizationResult | null = null;

  if (providerChain.length > 0) {
    chainResult = await runApiChain(providerChain, headlines, geoContext, lang, onProgress, 1, totalSteps);
  }
  if (chainResult) return chainResult;
