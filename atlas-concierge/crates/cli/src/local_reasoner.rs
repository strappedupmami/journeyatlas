...
pub struct TrainLocalReasonerArgs {
    ...
    #[arg(long, default_value = "../artifacts/local-reasoner")]
...
#[derive(Debug, Serialize)]
struct EvalMetricsArtifact {
    holdout_accuracy: f64,
    holdout_size: usize,
    label_metrics: Vec<LabelMetric>,
    confusion: BTreeMap<String, BTreeMap<String, usize>>,
}
...
pub fn train_and_write(args: TrainLocalReasonerArgs) -> Result<()> {
    ...
    if !(0.0..=1.0).contains(&args.min_holdout_accuracy) {
        bail!("--min-holdout-accuracy must be within [0.0, 1.0]");
    }

    let dataset_body = fs::read_to_string(&args.dataset)
        .with_context(|| format!("read local training dataset {}", args.dataset.display()))?;
    let dataset_sha256 = sha256_hex(dataset_body.as_bytes());

    let mut rows = parse_dataset(&dataset_body, &args.dataset)?;
    if rows.len() < 20 {
        bail!(
    ...
    let run_id = build_run_id(run_timestamp, args.run_tag.as_deref());
    ...
    let model = AppLocalReasoningModel {
        schema_version: 1,
        model_name: format!("atlas-local-reasoner-v3-{}", run_id),
        trained_at_utc: run_timestamp.to_rfc3339(),
    ...
    let run_dir = args.artifact_dir.join(&run_id);
    fs::create_dir_all(&run_dir)
        .with_context(|| format!("create run artifact dir {}", run_dir.display()))?;

    let model_artifact_path = run_dir.join("model.json");
    write_json_pretty(&model_artifact_path, &model)?;

    let eval_metrics = EvalMetricsArtifact {
        holdout_accuracy: eval.accuracy,
        holdout_size: eval.test_size,
        label_metrics: eval
            .per_label
            .iter()
            .map(|(label, (correct, total))| LabelMetric {
                label: label.clone(),
                correct: *correct,
                total: *total,
                accuracy: if *total == 0 {
    ...
    let metrics_artifact_path = run_dir.join("metrics.json");
    write_json_pretty(&metrics_artifact_path, &eval_metrics)?;

    let report_artifact_path = run_dir.join("report.md");
    fs::write(&report_artifact_path, &report).with_context(|| {
        format!(
            "write run report artifact {}",
            report_artifact_path.display()
        )
    })?;

    let threshold_passed = eval.accuracy >= args.min_holdout_accuracy;
    let mut output_files = BTreeMap::new();
    output_files.insert("model_json".to_string(), rel_string(&model_artifact_path));
    output_files.insert(
        "metrics_json".to_string(),
        rel_string(&metrics_artifact_path),
    );
    output_files.insert("report_md".to_string(), rel_string(&report_artifact_path));

    let manifest = RunManifest {
        run_id: run_id.clone(),
        run_tag: args.run_tag.clone(),
        generated_at_utc: run_timestamp.to_rfc3339(),
        dataset_path: rel_string(&args.dataset),
        dataset_sha256,
    ...
    let latest = LatestPointer {
        run_id: run_id.clone(),
        generated_at_utc: run_timestamp.to_rfc3339(),
        manifest_path: rel_string(&manifest_path),
        model_path: rel_string(&model_artifact_path),
        holdout_accuracy: eval.accuracy,
    };
    let latest_path = args.artifact_dir.join("latest.json");
    if let Some(parent) = latest_path.parent() {
        fs::create_dir_all(parent).with_context(|| {
            format!(
                "create artifact root for latest pointer {}",
                parent.display()
            )
        })?;
    }
    ...
    if !threshold_passed && !args.allow_below_threshold {
        bail!(
            "holdout accuracy {:.2}% below threshold {:.2}%. artifacts were generated at {}, but app model injection was blocked (use --allow-below-threshold to override)",
            eval.accuracy * 100.0,
            args.min_holdout_accuracy * 100.0,
            run_dir.display()
        );
    }
    ...
    println!(
        "trained local reasoner: run_id={} samples={} labels={} vocab={} eval_acc={:.2}% threshold_passed={}",
        run_id,
        rows.len(),
        model.labels.len(),
...
fn parse_dataset(body: &str, path: &Path) -> Result<Vec<TrainingRow>> {
...
fn evaluate_model(
    ...
    let mut by_label = BTreeMap::<String, Vec<TrainingRow>>::new();
    for row in rows {
        by_label
    ...
    let mut train = Vec::<TrainingRow>::new();
    let mut test = Vec::<TrainingRow>::new();
...
fn build_report(input: ReportInput, eval: &EvalSummary) -> String {
    ...
    if let Some(tag) = input.run_tag.as_ref() {
        report.push_str(&format!("- Run tag: `{}`\n", tag));
    }
    report.push_str(&format!("- Dataset: `{}`\n", input.dataset_path.display()));
    report.push_str(&format!("- Dataset SHA-256: `{}`\n", input.dataset_sha256));
    ...
    report.push_str(&format!(
        "- Holdout threshold: {:.2}%\n",
        input.min_holdout_accuracy * 100.0
    ));
    report.push_str(&format!(
        "- Threshold pass: {}\n\n",
        if eval.accuracy >= input.min_holdout_accuracy {
            "yes"
        } else {
            "no"
        }
    ...
    if eval.confusion.is_empty() {
        report.push_str("No confusion matrix available.\n");
    } else {
        for (actual, predicted_map) in &eval.confusion {
            let mut line = format!("- {} -> ", actual);
            let mut first = true;
            for (predicted, count) in predicted_map {
                if !first {
                    line.push_str(", ");
                }
                first = false;
                line.push_str(&format!("{}:{}", predicted, count));
            }
            line.push('\n');
...
fn build_run_id(now: chrono::DateTime<chrono::Utc>, run_tag: Option<&str>) -> String {
